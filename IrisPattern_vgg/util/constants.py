...
tf.compat.v1.flags.DEFINE_integer('WIDTH', 96,
                     'target spatial dimension 너비')
tf.compat.v1.flags.DEFINE_integer('HEIGHT', 96,
...
# tf.flags.DEFINE_integer('EPOCHS', 300,
#                      '같은 모이고사를 몇번 반복할것인지')
...
tf.compat.v1.flags.DEFINE_string('DATA_MODEL',
                    'multi-class_L2_MobileNetV2_albumentation',
                    '프로젝트 이름')

tf.compat.v1.flags.DEFINE_string('PROJECT_NAME',
                    FLG.DATA_MODEL + str(FLG.EPOCHS) + '_' + str(FLG.BATCH_SIZE),
                    '프로젝트 이름 : data + model + epoch + batch size ')


# validation result 저장 경로 '''
tf.compat.v1.flags.DEFINE_string('CIFAR10',
                    'D:/Data/cifar10',
                    '불러올 이미지 저장 경로')
